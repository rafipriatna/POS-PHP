<?php
    $rafi = $_GET['id'];
    // Mengubah data berdasarkan id kategori.
    $sql = $koneksi->query("SELECT * FROM tb_kategoribarang WHERE id_kategori = '$rafi'");
...
<h4 class="card-title">Tambah Kategori Barang</h4>
    ...
    <form method="POST">
        ...
        <label class="text-info control-label">Nama kategori</label>
            <div class="col-sm-10">
                <input type="text" name="nama_kategori" class="form-control" value="<?php echo $tampil['nama_kategori']?>"/>
            </div>
        </div>
        ...
        <label class="text-warning control-label">Deskripsi kategori</label>
            <div class="col-sm-10">
                <input type="text" name="deskripsi_kategori" class="form-control" value="<?php echo $tampil['deskripsi_kategori']?>"/>
            </div>
        </div>
    ...
    // Disimpan di tb_kategoribarang.
            $sql = $koneksi->query("UPDATE tb_kategoribarang SET nama_kategori='$nama_kategori', 
            deskripsi_kategori='$deskripsi_kategori' WHERE id_kategori='$rafi'");
